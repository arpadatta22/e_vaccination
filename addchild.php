...
<?php
	...
	?>
	<label ><b>Add Hospital</b></label></br>
<br>
<?php
	include 'database_connection.php';
...
<?php
	...
	if(isset($_POST['addchild']))
	{
		...
		$weight = $_POST['cweight'];
		$height = $_POST['cheight'];
		$hkval = implode(',',$_POST['hk']);
		
		
		
		$p_username = $_SESSION['username'];

		$query="INSERT INTO child VALUES('$name','$gen','$city','$birth',$age,$weight,$height,'$hkval','$p_username')";
		...
		$hospitalval = $_POST['hk'];
		$date = date('Y-m-d');
		for($i=0; $i<count($hospitalval); $i++)
		{
				$query_hospital = "SELECT * FROM hospital ";
				$result_hospital = mysqli_query($con,$query_hospital);

				while($row_hospital = mysqli_fetch_array($result_hospital))
				{
					if($hospitalval[$i] == $row_hospital['name'])
					{
						$time = strtotime($date);
						$final = date("Y-m-d", strtotime($row_hospital['timing'], $time));
						$query_date = "INSERT INTO hospital_dates VALUES('$hospitalval[$i]','$date','$final','$name','$p_username','false')";
